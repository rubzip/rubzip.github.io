import { safeGetCollection } from './content';

export async function getAllTags() {
    const publications = await safeGetCollection('publications');
    const talks = await safeGetCollection('talks');
    const projects = await safeGetCollection('projects');
    const posts = await safeGetCollection('posts');
    const teaching = await safeGetCollection('teaching');
